...
def block_quantize_4bit(x: torch.Tensor, group_size: int = 16):    
    ...
    # NOTE TO SELF (future ref): we are mapping 4 bits/val from [0,1] -> [0,15] or 2**k - 1 where k=4
    x_quant_8 = (x_norm * 15).round().to(torch.int8)
    # all rows and every other col for even and odd bits
...
class Linear4Bit(nn.Module):
    def __init__(self, in_features: int, out_features: int, bias: bool = True, group_size: int = 16):
        ...
        # grab the 2 weights that fit into the 8bit integer and store in tensor buffer
        ...
        # same, but tensor buffer for multipliers to help dequant step
        self.register_buffer(
            "weight_norm",
            torch.zeros(
        ...
        # recursive call to nn.Module to find all float32 weight tensors
